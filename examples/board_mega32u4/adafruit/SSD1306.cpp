...
#define digitalPinToPort(a)
#define digitalPinToBitMask(a) 0
#define portOutputRegister(a) ((volatile unsigned char*)nullptr)
...
void Adafruit_SSD1306::begin(uint8_t vccstate, uint8_t i2caddr, bool reset) {
  ...
  if ((reset) && (rst >= 0)) {
    ...
    DDRD |= 1<<PD2; //pinMode(rst, OUTPUT);
    PORTD |= 1<<PD2; //digitalWrite(rst, HIGH);
    // VDD (3.3V) goes high at start, lets just chill for a ms
    _delay_ms(1);
    // bring reset low
    PORTD &= ~(1<<PD2); //digitalWrite(rst, LOW);
    // wait 10ms
    _delay_ms(10);
    // bring out of reset
    PORTD |= 1<<PD2; //digitalWrite(rst, HIGH);
